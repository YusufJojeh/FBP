...
// Get all clients the vendor has orders with
$clients = [];
...
// AJAX: fetch chat messages with a client (across all bookings)
if (isset($_GET['chat']) && isset($_GET['client_id'])) {
    ...
    $query = "SELECT m.*, u.username, u.role
              ...
              WHERE (b.vendor_id = ? AND b.client_id = ?)
              ORDER BY m.sent_at ASC";
    $stmt = mysqli_prepare($conn, $query);
    mysqli_stmt_bind_param($stmt, 'ii', $vendor_id, $client_id);
    mysqli_stmt_execute($stmt);
    $result = mysqli_stmt_get_result($stmt);
    while ($row = mysqli_fetch_assoc($result)) {
        $messages[] = $row;
...
if (isset($_POST['send_chat']) && isset($_POST['client_id']) && isset($_POST['message'])) {
    $client_id = intval($_POST['client_id']);
    $message = trim($_POST['message']);
    $sender_id = $_SESSION['user_id'];
    $file = null;
    // Find a booking between this vendor and client (use the latest)
    $stmt = mysqli_prepare($conn, "SELECT id FROM bookings WHERE vendor_id = ? AND client_id = ? ORDER BY created_at DESC LIMIT 1");
    mysqli_stmt_bind_param($stmt, 'ii', $vendor_id, $client_id);
    mysqli_stmt_execute($stmt);
    $result = mysqli_stmt_get_result($stmt);
...
<head>
  ...
  <style>
    ...
    .client-info-list {flex:1;}
    .client-name {font-weight:700;color:#fff;}
    .client-email {font-size:.97rem;color:#ffd700;}
    ...
    @media (max-width:900px) {
      ...
      .clients-list {width:100%;max-height:180px;min-height:unset;display:flex;overflow-x:auto;overflow-y:hidden;}
      .client-item {flex-direction:column;align-items:center;min-width:180px;min-height:100px;}
...
<body>
  ...
  <section class="chats-hero">
    <h1>Chats</h1>
    <p>Chat with your clients in real time</p>
  </section>
  <div class="chats-container mt-4">
    <div class="clients-list" id="clientsList">
      <?php foreach ($clients as $i => $client): ?>
        <div class="client-item<?= $i===0 ? ' active' : '' ?>" data-client-id="<?= $client['id'] ?>" onclick="selectClient(this, <?= $client['id'] ?>, '<?= htmlspecialchars($client['username'], ENT_QUOTES) ?>')">
          ...
          <div class="client-info-list">
            ...
            <div class="last-msg-preview" id="preview-<?= $client['id'] ?>">&nbsp;</div>
          </div>
        </div>
      <?php endforeach; ?>
      <?php if (empty($clients)): ?>
        <div class="text-center text-muted p-4">No clients yet.</div>
      <?php endif; ?>
    </div>
    <div class="chat-area">
  ...
  <script>
    let currentClientId = <?= count($clients) ? $clients[0]['id'] : 'null' ?>;
    let chatInterval = null;
    function selectClient(el, clientId, clientName) {
    ...
    function fetchChatMessages() {
      ...
      fetch('?chat=1&client_id=' + currentClientId)
        .then(res => res.json())
        .then(messages => {
          const box = document.getElementById('chatMessages');
          if (!messages.length) {
            box.innerHTML = '<div class="text-center text-muted">No messages yet.</div>';
            document.getElementById('preview-' + currentClientId).textContent = '';
            return;
          }
          ...
          document.getElementById('preview-' + currentClientId).textContent = (last.message ? last.message : (last.file ? '[File]' : ''));
        });
    }
